	...
	class TravelController extends WebController
	{
		...
		public function edit(Request $request, Travel $travel): Response
		{
			...
			if ($form->isSubmitted() && $form->isValid()) {
				...
				$previuosTravellersNames = $this->repository
						->findById($travel->getId())
						->getTravellersNames();
				
				// Remove the relationship between the traveller name and the travel
				foreach ($previuosTravellersNames as $travellerName) {
					$this->repository->removeOldReferenceToTravellersNames($travellerName);
				}

				$resolution = $travel->getResolution();
				
				$fileNames = $this->fileManager->uploadFiles($travel->getResolution(),$request);
